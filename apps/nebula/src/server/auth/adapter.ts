import { accounts } from "@solstice/cosmos/tables/accounts";
import { sessions } from "@solstice/cosmos/tables/sessions";
import { NewUser, users } from "@solstice/cosmos/tables/users";
import { verificationTokens } from "@solstice/cosmos/tables/verification-tokens";
...
export function DrizzleAdapter(db: PostgresJsDatabase): Adapter {
  return {
    async createUser(userData) {
      //
      const newUser: Pick<NewUser, "id" | "email" | "emailVerified"> = {
        id: v4(),
    ...
    async updateUser({ id, ...userData }) {
      ...
      await db
        .update(users)
        .set({ ...userData, emailVerified: userData.emailVerified! })
        .where(eq(users.id, id));
      const rows = await db
        .select()
        .from(users)
